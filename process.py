...
data_1 = readData('orgs.csv')
data_2 = readData('eins.csv')
...
with open(output_file, 'w') as f:
    ...
    for fileno, filename in enumerate(('orgs.csv', 'eins.csv')) :
        with open(filename) as f_input :
            ...
            if header_unwritten :
                ...
                heading_row.insert(0, 'nid')
                heading_row.insert(0, 'our_name')
                heading_row.insert(0, 'irs_ein')
                heading_row.insert(0, 'irs_name')
            ...
            for row_id, row in enumerate(reader):
                ...
                if (cluster_id in group_ids):
                    print("Adding", cluster_id, group_ids[cluster_id], row)
                    group_ids[cluster_id].extend(row)
                else:
                    group_ids[cluster_id] = [score, row[3], row[4]]



    for group in group_ids.items():
        if (len(group[1]) > 3):
            writer.writerow(group[1])
            print(group)
